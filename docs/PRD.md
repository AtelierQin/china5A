...
*   **详情弹窗 (Pop-up)**：点击地图标记或列表项，弹出详情卡片。
    *   基础信息：名称、封面图（含智能占位图方案）、简介（基于 JSON 加载）。
    *   **智能图片方案 (Image Solution)**：
        *   **SVG 艺术占位**：对缺失图片的景点，立即生成包含省份/名称的几何艺术卡。
        *   **维基百科集成**：后台自动抓取维基百科缩略图。
        *   **数据缓存**：抓取到的图片 URL 存入 LocalStorage，实现二次开启秒加载。
    *   **操作栏**：[打卡/取消打卡]。
...
### 第三阶段：统计与优化 (已完成 ✅)
*   **智能图片系统**：实现 SVG 艺术占位 + 维基百科异步抓取 + 本地图片缓存。
*   移动端适配（手机浏览器打开体验良好）。
